...
class GameManager {
  ...
  init() {
    ...
    // Fullscreen Toggles (Title Action Bar & In-Game HUD)
    const titleFsBtn = document.getElementById('title-btn-fs');
    ...
    const showIPhoneToast = () => {
      ...
      this.scheduleTimeout(() => toast.classList.remove('visible'), 5000);
    };
    ...
    if (titleFsBtn) titleFsBtn.addEventListener('click', toggleFs);
    if (startFsBtn) startFsBtn.addEventListener('click', toggleFs);
    if (hudFsBtn) hudFsBtn.addEventListener('click', toggleFs);
